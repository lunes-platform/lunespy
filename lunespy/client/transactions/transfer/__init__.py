...
class TransferToken(BaseTransaction):
    def __init__(self, sender: Account, receiver: Account, fee: int = None, timestamp: int = None,
                 asset_fee:int = None, asset_id = None, amount:float = None) -> None:
        transfer_data: dict = drop_none({
            'timestamp': timestamp,
            'asset_fee': asset_fee,
            'asset_id': asset_id,
            'amount': amount,
