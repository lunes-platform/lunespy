...
from .protobuf.lunes import amount_pb2
...
class Address(object):
    def __init__(self, address='', publicKey='', privateKey='', seed='', alias='', nonce=0, lunespy=lunespy):
        ...
        if seed:
            ...
            if not self.lunespy.validateAddress(address):
                ...
                self.seed = seed
                self.nonce = nonce
        elif alias and not self.lunespy.OFFLINE:
            self.address = self.lunespy.wrapper('/alias/by-alias/%s' % alias).get("address", "")
            self.publicKey = ''
            self.privateKey = ''
            self.seed = ''
            self.nonce = 0
    ...
    def __str__(self):
        if self.address:
            ab = []
            try:
                assets_balances = self.lunespy.wrapper('/assets/balance/%s' % self.address)['balances']
    ...
    def balance(self, assetId='', confirmations=0):
        try:
            if assetId:
                return self.lunespy.wrapper('/assets/balance/%s/%s' % (self.address, assetId))['balance']
            else:
                return self.lunespy.wrapper(
    ...
    def transactions(self, limit=100, after=''):
        return self.lunespy.wrapper('/transactions/address/%s/limit/%d%s' % (
        self.address, limit, "" if after == "" else "?after={}".format(after)))

    def assets(self):
        req = self.lunespy.wrapper('/assets/balance/%s' % self.address)['balances']
        return [r['assetId'] for r in req]

    def aliases(self):
        a = self.lunespy.wrapper('/alias/by-address/%s' % self.address)
        if type(a) == list:
    ...
    def _generate(self, publicKey='', privateKey='', seed='', nonce=0):
        ...
        unhashedAddress = chr(1) + str(self.lunespy.CHAIN_ID) + crypto.hashChain(pubKey)[0:20]
    ...
    def issueAsset(self, name, description, quantity, decimals=0, reissuable=False,
                   txFee=lunespy.DEFAULT_ASSET_FEE):
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            self.lunespy.throw_error(msg)
            ...
            req = self.lunespy.wrapper('/assets/broadcast/issue', data)
            if self.lunespy.OFFLINE:
                return req
            else:
                return lunespy.Asset(req['assetId'], self.lunespy)
    ...
    def reissueAsset(self, Asset, quantity, reissuable=False, txFee=lunespy.DEFAULT_TX_FEE):
        ...
        req = self.lunespy.wrapper('/assets/broadcast/reissue', data)
        if self.lunespy.OFFLINE:
            return req
        else:
    ...
    def burnAsset(self, Asset, quantity, txFee=lunespy.DEFAULT_TX_FEE):
        ...
        req = self.lunespy.wrapper('/assets/broadcast/burn', data)
        if self.lunespy.OFFLINE:
            return req
        else:
            return req.get('id', 'ERROR')
    ...
    def sendLunes(self, recipient, amount, attachment='', txFee=lunespy.DEFAULT_TX_FEE, timestamp=0):
        if not self.privateKey:
            ...
            logging.error(msg)
            self.lunespy.throw_error(msg)
        elif not self.lunespy.OFFLINE and self.balance() < amount + txFee:
            msg = 'Insufficient Lunes balance'
            logging.error(msg)
            self.lunespy.throw_error(msg)
            ...
            return self.lunespy.wrapper('/transactions/broadcast', data)

    def massTransferLunes(self, transfers, attachment='', timestamp=0, baseFee=lunespy.DEFAULT_BASE_FEE):
        ...
        if not self.privateKey:
            ...
            logging.error(msg)
            self.lunespy.throw_error(msg)
        elif not self.lunespy.OFFLINE and self.balance() < totalAmount + txFee:
            msg = 'Insufficient Lunes balance'
            logging.error(msg)
            self.lunespy.throw_error(msg)
        else:
    ...
    def sendAsset(self, recipient, asset, amount, attachment='', feeAsset='', txFee=lunespy.DEFAULT_TX_FEE,
                  timestamp=0):
        if not self.privateKey:
            msg = 'Asset not issued'
            logging.error(msg)
            self.lunespy.throw_error(msg)
        elif not self.lunespy.OFFLINE and asset and not asset.status():
            msg = 'Asset not issued'
            logging.error(msg)
            self.lunespy.throw_error(msg)
        elif amount <= 0:
            ...
            logging.error(msg)
            self.lunespy.throw_error(msg)
        elif not self.lunespy.OFFLINE and asset and self.balance(asset.assetId) < amount:
            msg = 'Insufficient asset balance'
            logging.error(msg)
            self.lunespy.throw_error(msg)
        elif not self.lunespy.OFFLINE and not asset and self.balance() < amount:
            msg = 'Insufficient Lunes balance'
            logging.error(msg)
            self.lunespy.throw_error(msg)
        elif not self.lunespy.OFFLINE and not feeAsset and self.balance() < txFee:
            msg = 'Insufficient Lunes balance'
            logging.error(msg)
            self.lunespy.throw_error(msg)
        elif not self.lunespy.OFFLINE and feeAsset and self.balance(feeAsset.assetId) < txFee:
            msg = 'Insufficient asset balance'
            logging.error(msg)
            self.lunespy.throw_error(msg)
        else:
            if feeAsset:
                feeInfos = self.lunespy.wrapper('/assets/details/' + feeAsset.assetId)
            ...
            return self.lunespy.wrapper('/assets/broadcast/transfer', data)

    def massTransferAssets(self, transfers, asset, attachment='', timestamp=0, baseFee=lunespy.DEFAULT_BASE_FEE,
                           smartFee=lunespy.DEFAULT_SMART_FEE):
        txFee = baseFee + (math.ceil((len(transfers) + 1) / 2 - 0.5)) * baseFee
        ...
        elif not self.lunespy.OFFLINE and self.balance() < txFee:
            logging.error('Insufficient Lunes balance')
        elif not self.lunespy.OFFLINE and self.balance(assetId=asset.assetId) < totalAmount:
    ...
    '''def dataTransaction(self, data, timestamp=0, baseFee=lunespy.DEFAULT_BASE_FEE, minimalFee=500000):
        if not self.privateKey:
            logging.error('Private key required')
        else:
            if timestamp == 0:
            ...
            return self.lunespy.wrapper('/transactions/broadcast', dataObjectJSON)'''
    ...
    def dataTransaction(self, data, timestamp=0, baseFee=lunespy.DEFAULT_BASE_FEE, minimalFee=500000):
        ...
        transaction.chain_id = ord(self.lunespy.CHAIN_ID)
        ...
        jsonMessage['chainId'] = ord(self.lunespy.CHAIN_ID)
        jsonMessage['version'] = 2
        jsonMessage['data'] = dataList

        req = self.lunespy.wrapper('/transactions/broadcast', json.dumps(jsonMessage))

        return req

    def deleteDataEntry(self, key, timestamp=0, baseFee=lunespy.DEFAULT_BASE_FEE, minimalFee=500000):
        dataTransaction = transaction_pb2.DataTransactionData()
    ...
    def _postOrder(self, amountAsset, priceAsset, orderType, amount, price, maxLifetime=30 * 86400, matcherFee=lunespy.DEFAULT_MATCHER_FEE, timestamp=0):
        ...
        sData = b'\3' + \
                base58.b58decode(self.publicKey) + \
                base58.b58decode(self.lunespy.MATCHER_PUBLICKEY) + \
                asset1 + \
                asset2 + \
                orderType + \
                struct.pack(">Q", price) + \
                ...
                b'\0'
        signature = crypto.sign(self.privateKey, sData).decode()
        otype = "buy" if orderType == b'\0' else "sell"
        data = json.dumps({
            "senderPublicKey": self.publicKey,
            "matcherPublicKey": self.lunespy.MATCHER_PUBLICKEY,
            "assetPair": {
        ...
        req = self.lunespy.wrapper('/matcher/orderbook', data, host=self.lunespy.MATCHER)
        id = -1
        if 'status' in req:
        ...
        elif not self.lunespy.OFFLINE:
            logging.error(req)
            self.lunespy.throw_error(req)
        else:
            return req
        return id
    ...
    def cancelOrder(self, assetPair, order):
        ...
        req = self.lunespy.wrapper('/matcher/orderbook/%s/%s/cancel' % (
        self.lunespy.DEFAULT_CURRENCY if assetPair.asset1.assetId == '' else assetPair.asset1.assetId,
        self.lunespy.DEFAULT_CURRENCY if assetPair.asset2.assetId == '' else assetPair.asset2.assetId), data,
                                   host=self.lunespy.MATCHER)
        if self.lunespy.OFFLINE:
            return req
        else:
            id = -1
    ...
    def buy(self, assetPair, amount, price, maxLifetime=30 * 86400, matcherFee=lunespy.DEFAULT_MATCHER_FEE,
            timestamp=0):
        assetPair.refresh()
        ...
        if self.lunespy.OFFLINE:
            ...
            return self.lunespy.Order(id, assetPair, self)

    def sell(self, assetPair, amount, price, maxLifetime=30 * 86400, matcherFee=lunespy.DEFAULT_MATCHER_FEE,
             timestamp=0):
        assetPair.refresh()
        #normPrice = int(pow(10, 8 + assetPair.asset2.decimals - assetPair.asset1.decimals) * price)
        normPrice = int(round(pow(10, 8 + assetPair.asset2.decimals - assetPair.asset1.decimals) * price))
    ...
    def tradableBalance(self, assetPair):
        try:
            req = self.lunespy.wrapper('/matcher/orderbook/%s/%s/tradableBalance/%s' % (
            self.lunespy.DEFAULT_CURRENCY if assetPair.asset1.assetId == '' else assetPair.asset1.assetId,
            self.lunespy.DEFAULT_CURRENCY if assetPair.asset2.assetId == '' else assetPair.asset2.assetId,
            self.address), host=self.lunespy.MATCHER)
            if self.lunespy.OFFLINE:
                return req
            amountBalance = req[
                self.lunespy.DEFAULT_CURRENCY if assetPair.asset1.assetId == '' else assetPair.asset1.assetId]
    ...
    def lease(self, recipient, amount, txFee=lunespy.DEFAULT_LEASE_FEE, timestamp=0):
        if not self.privateKey:
            ...
            req = self.lunespy.wrapper('/transactions/broadcast', data)
            return req
    ...
    '''def lease(self, recipient, amount, txFee=lunespy.DEFAULT_LEASE_FEE, timestamp=0):
        if not self.privateKey:
            ...
            req = self.lunespy.wrapper('/leasing/broadcast/lease', data)
            return req'''
    ...
    def leaseCancel(self, leaseId, txFee=lunespy.DEFAULT_LEASE_FEE, timestamp=0):
        if not self.privateKey:
            ...
            req = self.lunespy.wrapper('/leasing/broadcast/cancel', data)
            if self.lunespy.OFFLINE:
                return req
            elif 'leaseId' in req:
                return req['leaseId']
    ...
    def getOrderHistory(self, assetPair, timestamp=0):
        ...
        req = self.lunespy.wrapper('/matcher/orderbook/%s/%s/publicKey/%s' % (
        self.lunespy.DEFAULT_CURRENCY if assetPair.asset1.assetId == '' else assetPair.asset1.assetId,
        self.lunespy.DEFAULT_CURRENCY if assetPair.asset2.assetId == '' else assetPair.asset2.assetId, self.publicKey),
                                   headers=data, host=self.lunespy.MATCHER)
        return req
    ...
    def cancelOpenOrders(self, assetPair):
        ...
        for order in orders:
            ...
            if status == 'Accepted' or status == 'PartiallyFilled':
                ...
                self.lunespy.wrapper('/matcher/orderbook/%s/%s/cancel' % (
                self.lunespy.DEFAULT_CURRENCY if assetPair.asset1.assetId == '' else assetPair.asset1.assetId,
                self.lunespy.DEFAULT_CURRENCY if assetPair.asset2.assetId == '' else assetPair.asset2.assetId), data,
                                     host=self.lunespy.MATCHER)
    ...
    def deleteOrderHistory(self, assetPair):
        ...
        for order in orders:
            ...
            self.lunespy.wrapper('/matcher/orderbook/%s/%s/delete' % (
            self.lunespy.DEFAULT_CURRENCY if assetPair.asset1.assetId == '' else assetPair.asset1.assetId,
            self.lunespy.DEFAULT_CURRENCY if assetPair.asset2.assetId == '' else assetPair.asset2.assetId), data,
                                 host=self.lunespy.MATCHER)

    def createAlias(self, alias, txFee=lunespy.DEFAULT_ALIAS_FEE, timestamp=0):
        aliasWithNetwork = b'\x02' + crypto.str2bytes(str(self.lunespy.CHAIN_ID)) + struct.pack(">H", len(
        ...
        if not self.privateKey:
            ...
            return self.lunespy.wrapper('/alias/broadcast/create', data)
    ...
    def setScript(self, scriptSource, txFee=lunespy.DEFAULT_SCRIPT_FEE, timestamp=0):
        script = self.lunespy.wrapper('/utils/script/compile', scriptSource)['script'][7:]
        if not self.privateKey:
            logging.error('Private key required')
        else:
            ...
            sData = b'\x0d' + \
                    b'\1' + \
                    crypto.str2bytes(str(self.lunespy.CHAIN_ID)) + \
                    base58.b58decode(self.publicKey) + \
                    b'\1' + \
    ...
    def setAssetScript(self, asset, scriptSource, txFee=lunespy.DEFAULT_ASSET_SCRIPT_FEE, timestamp=0):
        script = self.lunespy.wrapper('/utils/script/compile', scriptSource)['script'][7:]
        if not self.privateKey:
            logging.error('Private key required')
        else:
    ...
    def invokeScript(self, dappAddress, functionName, params, payments, feeAsset=None,
                     txFee=lunespy.DEFAULT_INVOKE_SCRIPT_FEE):
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            self.lunespy.throw_error(msg)
