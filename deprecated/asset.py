...
class Asset(object):
    ...
    def status(self):
        if self.assetId!=lunespy.DEFAULT_CURRENCY:
            try:
                req = self.lunespy.wrapper('/assets/details/%s' % self.assetId)
                if req['assetId'] != None:
    ...
    def isSmart(self):
        req = self.lunespy.wrapper('/transactions/info/%s' % self.assetId)
...
class AssetPair(object):
    def __init__(self, asset1, asset2, lunespy=lunespy):
        self.lunespy = lunespy
        self.asset1 = asset1
        self.asset2 = asset2
        self.a1 = lunespy.DEFAULT_CURRENCY if self.asset1.assetId == '' else self.asset1.assetId
        self.a2 = lunespy.DEFAULT_CURRENCY if self.asset2.assetId == '' else self.asset2.assetId
    ...
    def orderbook(self):
        req = self.lunespy.wrapper('/matcher/orderbook/%s/%s' % (self.a1, self.a2), host=self.lunespy.MATCHER)
        return req

    def ticker(self):
        return self.lunespy.wrapper('/v0/pairs/%s/%s' % (self.a1, self.a2), host=self.lunespy.DATAFEED)
    ...
    def _getMarketData(self, method, params):
        return self.lunespy.wrapper('%s/%s/%s/%s' % (method, self.a1, self.a2, params), host=self.lunespy.DATAFEED)

    def trades(self, *args):
        if len(args)==1:
            limit = args[0]
            if limit > 0 and limit <= self.lunespy.MAX_WDF_REQUEST:
    ...
    def candles(self, *args):
        if len(args)==2:
            timeframe = args[0]
            limit = args[1]
            if timeframe not in self.lunespy.VALID_TIMEFRAMES:
                msg = 'Invalid timeframe'
                self.lunespy.throw_error(msg)
                return logging.error(msg)
            elif limit > 0 and limit <= self.lunespy.MAX_WDF_REQUEST:
